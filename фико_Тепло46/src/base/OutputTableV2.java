...
public class OutputTableV2
{
	...
	public OutputTableV2(String year, String pathfile)
	{
		...
		Vector<Vector<String>> para = new ConnectionBD().getOutputTableV2_par(year);
		Vector<Vector<String>> gor = new ConnectionBD().getOutputTableV2_gor(year);
		...
		try
		{
			...
			sheet_par.addCell(new Label(0, 0, "Полезный отпуск ТЭ в паре.", cellFormat));

			sheet_gor.addCell(new Label(0, 0, "Полезный отпуск ТЭ в горячей воде.", cellFormat));
			...
			WritableCellFormat cellFormat5 = new WritableCellFormat(tahoma9ptBold);
			// выравнивание по центру
			...
			for (int i = 0; i < para.size(); i++)
			{
				...
				{
					// не тру((ячейка белая)
					sheet_par.addCell(new Label(p, 3 + i, para.get(i).get(p).toString(), cellFormat4));
					sheet_par.addCell(new Label(p+1, 3 + i, para.get(i).get(p+1).toString(), cellFormat3));
				}
				
				{
					...
					{
						sheet_par.addCell(new Label(p, 3 + i, Double.toString(itog), cellFormat5));
						sheet_par.addCell(new Label(p+1, 3 + i, Double.toString(narast), cellFormat5));
					}else
					{
						sheet_par.addCell(new Label(p, 3 + i, Double.toString(itog), cellFormat4));
						sheet_par.addCell(new Label(p+1, 3 + i, Double.toString(narast), cellFormat3));
					}
				}
				...
				l1 = new Label(0, 3 + i, para.get(i).get(0).toString(), cellFormat4);
				...
				for (int p = 0; p < districts.length; p++)
				{
					...
					if (districts[p].equals(para.get(i).get(0)))
					{
						...
						{
							sheet_par.addCell(new Label(z, 3 + i, "", cellFormat2));
						}
						l1 = new Label(0, 3 + i, para.get(i).get(0).toString(), cellFormat2);
						break;
					}
				}
			...
			for (int i = 0; i < gor.size(); i++)
			{
				...
				{
					// не тру((ячейка белая)
					sheet_gor.addCell(new Label(p, 3 + i, gor.get(i).get(p).toString(), cellFormat4));
					sheet_gor.addCell(new Label(p+1, 3 + i, gor.get(i).get(p+1).toString(), cellFormat3));
				}
				
				{
					...
					if(itog.equals(narast)!= true)
					{
						sheet_gor.addCell(new Label(p, 3 + i, Double.toString(itog), cellFormat5));
						sheet_gor.addCell(new Label(p+1, 3 + i, Double.toString(narast), cellFormat5));
					}else
					{
						sheet_gor.addCell(new Label(p, 3 + i, Double.toString(itog), cellFormat4));
						sheet_gor.addCell(new Label(p+1, 3 + i, Double.toString(narast), cellFormat3));
					}
				}
				...
				// добавление организаций
				l1 = new Label(0, 3 + i, gor.get(i).get(0).toString(), cellFormat4);
				...
				for (int p = 0; p < districts.length; p++)
				{
					...
					if (districts[p].equals(gor.get(i).get(0)))
					{
						...
						{
							sheet_gor.addCell(new Label(z, 3 + i, "", cellFormat2));
						}
						l1 = new Label(0, 3 + i, gor.get(i).get(0).toString(), cellFormat2);
						break;
					}
				}
	...
	private WritableSheet shapka(WritableSheet sheet_par)
	{
		try
		{
			...
			sheet_par.addCell(new Label(0, 1, "Организация (всего за мес./ всего с нарастающим итогом в Тыс. Гкал)", cellFormat));
			...
			sheet_par.addCell(new Label(26, 2, "С нарастающим итогом", cellFormat));

			for (int i = 0; i < 12; i++)
			{
				sheet_par.addCell(new Label(1 + i * 2, 2, "за месяц", cellFormat));
				sheet_par.addCell(new Label(2 + i * 2, 2, "С нарастающим итогом", cellFormat));
			}
